...
export const Default: Story = {
  args: { size: 24, color: 'default', variant: 'outlined', filled: false },
  render: (args, context) => (
    <Icon {...args} name={(context.globals.iconName as string) || args.name || 'home'} />
  ),
};
...
export const Variants: Story = {
  ...
  render: (_, context) => {
    const name = (context.globals.iconName as string) || 'home';
    return (
      <div style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            <Icon name={name} variant={variant} filled={filled} size={32} />
        ...
        {[
          ...
          return (
            <div key={color} style={{
              ...
              <Icon name={name} size={24} color={color as any} />
              <div>
...
export const IconGrid: Story = {
  name: 'Icon Grid',
  render: (_, context) => {
    const active = (context.globals.iconName as string) || 'home';
    return (
          ...
          {SAMPLE_ICONS.map(name => {
            ...
            return (
              <div key={name} style={{
                ...
                padding: '12px 8px',
                border: `1px solid ${isActive ? 'var(--ds-color-brand-default)' : 'var(--ds-color-border-subtle)'}`,
                borderRadius: 'var(--ds-radius-8)',
                width: 80,
                background: isActive ? 'var(--ds-color-brand-subtle)' : 'var(--ds-color-background-default)',
              }}>
                <Icon name={name} size={24} color={isActive ? 'brand' : 'default'} />
